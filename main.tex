...
Os dados buscados para cada patente foram: URL, título, data de publicação, citações da patente (\textit{patent citations}), citado por (\textit{cited by}), resumo (\textit{abstract}), descrição (\textit{description}), quantidade de imagens, documentos similares (\textit{similar documents}), aplicações que reivindicam prioridade (\textit{application claiming priority}), eventos legais (\textit{legal events}), conceitos (\textit{concepts}), inventores (\textit{authors}), outras línguas (\textit{other languages}), info e links externos (\textit{external links}). No entanto, devido à idade de algumas patentes e à sua disponibilidade na internet, o conjunto final de patentes foi reduzido a 5376 patentes.
...
Em seguida, fizemos testes com uma outra definição de patentes promissoras.
Usamos a definição usada por \textcite{kyebambe2017forecasting}: são consideradas promissoras as patentes que possuem pelo menos um código IPC mais recente do que a própria data de publicação da patente. A premissa é que essas patentes têm maior potencial de avanço tecnológico, pois introduzem conceitos ou tecnologias que foram formalmente reconhecidos em classificações posteriores.

Utilizamos essa definição para rotular as patentes do conjunto de treinamento e repetimos o processo de treinamento e avaliação dos modelos Random Forest e SVM, conforme descrito anteriormente, além de termos testado também um treinamento puramente supervisionado. Essa abordagem alternativa visa explorar diferentes critérios de avaliação para identificar patentes promissoras, permitindo uma comparação entre os resultados obtidos com as duas definições distintas.
...
\begin{table}[H]
        ...
        \bottomrule
        \end{tabular}
        \caption{Desempenho dos modelos SVM e Random Forest com a nova definição de patente promissora (Conjunto 4115 patentes)}
\end{table}
